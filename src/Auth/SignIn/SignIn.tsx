...
//this is captcha try
    const captcha= ['E42wS4','a2N56r','BH6EgT','Ut67Hy','hd8Hp2','qm38Ju','bU8KTy','Uoij78','kjdcn4','ZPok8c','hvu5vg','Uy87bj','UY899o','97heJH','KJ9hj7','99kjND','UWH8h8','HJiumn'];
    const [captaIndex,setCaptaIndex]=useState(0);
    const handleClick =()=>{
        setCaptaIndex(captaIndex+1);
        if(captaIndex >= captcha.length){
            setCaptaIndex(0)
        }
    };
    //end
        ...
        <>
            <div className='w-screen h-screen bg-[#9c27b0] relative flex justify-center items-center'>
                <div className='w-[80%] max-w-[70rem] max-sm:w-full max-sm:h-full max-[900px]:flex-col  absolute rounded-md bg-white shado flex overflow-y-auto'>
                    ...
                    <div className='w-[40%] max-[900px]:w-full px-16 py-10 max-sm:h-auto border-r'><img className='w-full h-full' src={logo} alt="" /></div>
                    <Formik
                    ...
                    <Form className='w-[60%] max-[900px]:w-full p-8 flex  flex-col items-center'>
                        ...
                        <div className=' w-[18rem] max-sm:w-full mt-3  mb-10'>
                            ...
                            <input type='text' name='captcha' id='captcha' className='w-full h-full p-2 px-5  text-base font-serif font-medium outline-none text-center rounded-lg' placeholder='captcha code'/>
                            
                            <p className='font-serif text-red-600'><ErrorMessage name="captcha" /></p>
                            </div>
                        </div>
                        <div className='w-full text-gray-700 flex flex-col gap-4'>
                            <div className='flex items-center gap-5'>
                                <span><input type="checkbox" onClick={()=>setToggleLogin(!toggleLogin)} name="" id="" /></span>
                            ...
                            <div className='flex max-sm:flex-col max-sm:gap-5 justify-between'>
                                ...
                                <div className='max-sm:w-full w-[8rem] h-[2.25rem] rounded-md shadow-md border'><button disabled={!toggleLogin} type='submit' className={`${toggleLogin ? 'bg-green-600 text-white' : 'cursor-not-allowed'} rounded-md w-full h-full font-medium`}>Login</button></div>
                            </div>
                        </div>
